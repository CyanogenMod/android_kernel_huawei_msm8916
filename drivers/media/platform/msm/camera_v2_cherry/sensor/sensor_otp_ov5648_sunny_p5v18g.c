																...
																struct ov5648_sunny_p5v18g_otp_struct * otp_ptr)
{
	...
	pr_debug("%s,group 2 otp_ptr.product_year:%d\n",__func__,otp_ptr->product_year);
	pr_debug("%s,group 2 otp_ptr.product_month:%d\n",__func__,otp_ptr->product_month);
	pr_debug("%s,group 2 otp_ptr.product_date:%d\n",__func__,otp_ptr->product_date);
	pr_debug("%s,group 2 otp_ptr.camera_id:%d\n",__func__,otp_ptr->camera_id);
	pr_debug("%s,group 2 otp_ptr.supplier_version_id:%d\n",__func__,otp_ptr->supplier_version_id);

	ov5648_sunny_p5v18g_otp_clear(s_ctrl);

	if (0 != otp_ptr->product_year)// year is not zero, we assume module info is correct
	{
		pr_warn("%s, Module info lays in group 2\n",__func__);
		return 0;
	}
	...
	if (0 != otp_ptr->product_year) // year is not zero, we assume module info is correct
	{
		pr_warn("%s, Module info lays in group 1\n",__func__);
		return 0;//otp read sucess
	}
	else
	{
		pr_err("%s,  Error !No Module inf found in OTP\n",__func__);
												...
												struct ov5648_sunny_p5v18g_otp_struct * otp_ptr)
{
	...
	{
		pr_warn("%s, AWB info lays in group 1\n",__func__);
		return 0;//otp read sucess
	}
	else
	{
		pr_err("%s,  Error! No AWB info found in OTP\n",__func__);
		return -1;//otp read fail
	}
	return 0;
}
														...
														struct ov5648_sunny_p5v18g_otp_struct * otp_ptr)
{
	...
	if ((0 != vcm_start_value) && (0 != vcm_end_value))
	{
		...
		pr_debug("%s, VCM info read sucess!\n\n",__func__);
	...
	{
		pr_err("%s,  Error VCM info is not correct!\n",__func__);
											...
											struct ov5648_sunny_p5v18g_otp_struct * current_otp)
{
	...
	pr_debug("%s, ov5648_sunny_p5v18g_otp_read_flag:0x%x\n",__func__, ov5648_sunny_p5v18g_otp_read_flag);
	return;
}
...
static int ov5648_sunny_p5v18g_update_otp(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	{
		pr_err("%s, Error Zero rg or bg, rg:%d, bg:%d\n", __func__, rg, bg);
		return -1;
	}

	pr_debug("%s, r/g:0x%x, b/g:0x%x\n", __func__, rg, bg);
	//calculate G gain
	//0x400 = 1x gain
