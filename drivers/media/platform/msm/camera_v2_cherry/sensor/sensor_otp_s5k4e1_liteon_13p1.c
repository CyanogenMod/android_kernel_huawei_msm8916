...
void s5k4e1_liteon_otp_debug(struct s5k4e1_liteon_otp_struct  otp_ptr)
{
	...
	pr_debug("%s,otp_ptr.vcm_start:0x%x",__func__,otp_ptr.vcm_start);
	pr_debug("%s,otp_ptr.vcm_end:0x%x",__func__,otp_ptr.vcm_end);
	return;
}
/******************************************************************************
...
s5k4e1_liteon_otp_location s5k4e1_liteon_check_awb_otp_location(struct msm_sensor_ctrl_t * s_ctrl)
{
	...
	pr_debug("%s read layer 18: reg310d:0x%x, reg310e:0x%x, reg310f:0x%x \n", __func__, reg310d, reg310e, reg310f);
	...
	for (vcm_location = S5K4E1_LITEON_OTP_310D_LSB ; vcm_location <= S5K4E1_LITEON_OTP_310F_LSB; vcm_location++)
	{
		switch (vcm_location)
		{
			...
			case S5K4E1_LITEON_OTP_310F_LSB://reg 0x310f lsb
				...
				break;
			default:
				pr_err("%s Error vcmLocation %d", __func__, vcm_location);
				return;
		}
		pr_debug("%s reg=0x%x mask=%d", __func__, reg, mask);
		...
		for (layer = S5K4E1_LITEON_LAYER_VCM_START; layer <= S5K4E1_LITEON_LAYER_VCM_END; layer++)
		{
			...
			pr_debug("%s reg_values[%d]:0x%x, reg:%x", __func__, i, reg_values[i], reg);
		}
		...
		{
			pr_debug("%s VCM_Otp found, location:%d ", __func__, vcm_location);
			pr_debug("%s vcm_start:%d vcm_end:%d", __func__, current_otp->vcm_start, current_otp->vcm_end);
			//found vcm otp
		...
		{
			//error otp
			pr_err("%s Error! Invalide VCM Otp", __func__);
			return;
		}
	}

	pr_err("%s Error! No VCM Otp \n", __func__);
	return;
}
