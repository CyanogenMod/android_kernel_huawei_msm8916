...
static int cyttsp5_parse_input(struct cyttsp5_core_data *cd)
{
	...
	if((report_id == HID_TOUCH_REPORT_ID) && (cd->input_buf[5] & 0x80)) {
		tp_log_err( "%s: Esd error, report id:%X, buff[5]=%X\n", __func__, report_id, cd->input_buf[5]);
...
static int cyttsp5_startup_(struct cyttsp5_core_data *cd, bool reset)
{
...
#if defined (CONFIG_HUAWEI_DSM)
	g_tp_dsm_info.constraints_ESD_status = ESD_ERROR_NO_ERROR;
	...
	if(cd->transient_state) {
		tp_log_err( "%s: IC will be reset because of esd \n", __func__);
#if defined (CONFIG_HUAWEI_DSM)
		g_tp_dsm_info.constraints_ESD_status = ESD_ERROR_GHOUL_HAND;
#endif /* CONFIG_HUAWEI_DSM */
...
#if defined (CONFIG_HUAWEI_DSM)
		/* 1¡¢if this is not initiative reset, report dsm error
		 * 2¡¢if this is not the first reset, report dsm error 
		 * 3¡¢the first initiative reset will not report dsm error */
		if (!reset || retry != CY_CORE_STARTUP_RETRY_COUNT) {
			tp_log_err("%s %d: report esd dmd error!\n", __func__, __LINE__);
			g_tp_dsm_info.constraints_ESD_status = ESD_ERROR_NO_ACK;
		}
#endif /* CONFIG_HUAWEI_DSM */
		/* reset hardware */
...
#endif
#if defined (CONFIG_HUAWEI_DSM)
	if (ESD_ERROR_NO_ERROR != g_tp_dsm_info.constraints_ESD_status) {
		if (!dsm_client_ocuppy(tp_cyp_dclient)) {
			...
			dsm_client_notify(tp_cyp_dclient, DSM_TP_ESD_ERROR_NO);
		} else {
